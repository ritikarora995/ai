...
#102053036
import math
def search(mat_RITIKARORA, x):
    for i in range(len(mat_RITIKARORA)):
        row = mat_RITIKARORA[i]
        for j in range(len(row)):
            e = mat_RITIKARORA[i][j]
            if(e==x):
                return (i, j)
    return (-1, -1)
...
FOUND = False
fNode = None
while(q):  # HILL CLIMB, ALWAYS GET BEST SOLN
    if(q.state == finBoard):
        FOUND = True
        fNode = q
        break
    ...
    q = max(c, key=lambda item: item.h)
